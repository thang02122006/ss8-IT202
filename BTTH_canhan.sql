...
select  g.guest_id ,g.guest_name, count(b.booking_id) as thue_tren_2_lan
from guests g
join bookings b on b.guest_id = g.guest_id
group by b.guest_id
having thue_tren_2_lan >= 2;

select r.room_id, r.room_type, count(b.booking_id) as rooms_dat_nhieu
from rooms r
join bookings b on b.room_id = r.room_id
group by b.room_id
-- having max(rooms_dat_nhieu)